...
class TestLookaheadFeatures:
    """Tests for look-ahead features (signature territory detection)."""

    def test_name_pattern_below_detected(self) -> None:
        """Lines before a name pattern have has_name_pattern_below=True."""
        text = "本文です。\n\n田中太郎"
        result = _extract_features(text)

        # First line should see name pattern below
        assert result.line_features[0].has_name_pattern_below is True
        # Name line itself has no name below
        assert result.line_features[2].has_name_pattern_below is False

    def test_contact_info_below_detected(self) -> None:
        """Lines before contact info have has_contact_info_below=True."""
        text = "本文です。\n\nTEL: 03-1234-5678"
        result = _extract_features(text)

        # First line should see contact info below
        assert result.line_features[0].has_contact_info_below is True
        # Contact line itself has no contact below
        assert result.line_features[2].has_contact_info_below is False

    def test_closing_pattern_below_detected(self) -> None:
        """Lines before a closing have has_closing_pattern_below=True."""
        text = "本文です。\n\nよろしくお願いいたします。"
        result = _extract_features(text)

        # First line should see closing pattern below
        assert result.line_features[0].has_closing_pattern_below is True
        # Closing line itself has no closing below
        assert result.line_features[2].has_closing_pattern_below is False

    def test_signature_territory_detection(self) -> None:
        """Full signature block triggers look-ahead features on earlier lines."""
        text = """本文です。
...
TEL: 03-1234-5678"""
        ...
        # Body line (index 0) should see all signature signals below
        body_features = result.line_features[0]
        assert body_features.has_closing_pattern_below is True
        assert body_features.has_name_pattern_below is True
        assert body_features.has_contact_info_below is True

    def test_no_signature_no_lookahead(self) -> None:
        """Email without signature has no look-ahead signals."""
        text = "お世話になっております。\n本文です。\n続きです。"
        result = _extract_features(text)

        # No lines have signature-related patterns below
        for features in result.line_features:
            assert features.has_name_pattern_below is False
            assert features.has_contact_info_below is False

    def test_lookahead_helps_blank_line_context(self) -> None:
        """Blank lines between signature lines inherit look-ahead context."""
        text = """よろしくお願いいたします。

山田太郎"""
        result = _extract_features(text)

        # Blank line (index 1) should see name pattern below
        blank_features = result.line_features[1]
        assert blank_features.has_name_pattern_below is True
        # Closing line (index 0) should also see name pattern below
        assert result.line_features[0].has_name_pattern_below is True
